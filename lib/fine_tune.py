...
##################################################
## Lower
def model_builder_cnn_lower(hp, conv_dropout=True):
    ...
    # convulational layers
    conv_layers = hp.Int('conv_blocks', 2, 5, 
                         default=3)

    # dropout rate
    dropout_rate = hp.Float('dropout', 0, 0.4, 
                            step=0.1, default=0.2)
#     # activation
#     activation=hp.Choice("activation", ["relu", "tanh"])
...
def tune_train_best_model_low(bodypart,
                            valid_generators,
                            train_generators,
                            test):
    ...
    classes = 2
    tuner = kt.Hyperband(
                        model_builder_cnn_lower,
                        objective='val_binary_accuracy',
                        max_epochs=10,
                        hyperband_iterations=2,
                        directory='data/tuner/low'+bodypart,
                        project_name='finetune_low'
    )
    ...
    # perfrom search for best parameters
    validation_steps = math.ceil(valid_generators[bodypart].n/ (2*valid_generators[bodypart].batch_size))
    print("Using validation_steps = %d" % validation_steps)
    steps_per_epoch = math.ceil(train_generators[bodypart].n / (10*train_generators[bodypart].batch_size))
    print("Using steps_per_epoch = %d" % steps_per_epoch)
    print('Tuner search for {}.\n'.format(bodypart))
    tuner.search(train_generators[bodypart],
                  validation_data=valid_generators[bodypart],
...
#     best_hps=tuner.get_best_hyperparameters(num_trials=1)[0]
    ...
    # instantiate the hypermodel 
    # train it with the optimal number of epochs
    print('Train model for best epoch for {}.\n'.format(bodypart))
    hypermodel = tuner.hypermodel.build(best_hyperparams[bodypart])
    # plot summary
    hypermodel.summary()
    ...
    histories_best[bodypart] = hypermodel.fit(train_generators[bodypart],
                  ...
                  validation_steps = validation_steps,
                  steps_per_epoch=steps_per_epoch,
                  epochs=best_epochs[bodypart], 
                  callbacks=[stop_early])
    print('\nSave training history\n')
    with open("data/history/history_fine_tuned_best_model_low" + bodypart + ".json", "w") as file:
            json.dump(histories_best[bodypart].history, file)
    print('\nSave hypermodel\n')
    hypermodel.save("data/models/_fine_tuned_best_model_low" + bodypart)
    hypermodel.save("data/models/_fine_tuned_best_model_low" + bodypart + ".h5")
    print('\nEvaluate hypermodel\n')
    eval_func(test[test['bodypart'] == bodypart]['class'], 
                    hypermodel.predict(test_generators[bodypart]))
    hypermodel.evaluate(test_generators[bodypart], verbose=1)
    print("\nDelete model and hypermodel")    
    clean_up(model)     
    clean_up(hypermodel)        
    return tuners, best_hyperparams, best_epochs, histories
